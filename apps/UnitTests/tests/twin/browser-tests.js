...
// Define test suite
const UnitTestsBrowserTests = {
    suiteName: 'UnitTests Browser Tests',
    version: '1.0.0',
    author: 'PSWebHost',
    ...
    async testComponentLoading() {
        const testElement = document.createElement('unittests-home');
    ...
    async testAPIEndpoint() {
        const response = await fetch('/apps/UnitTests/api/v1/status');
    ...
    async testUIElementRendering() {
        const response = await fetch('/apps/UnitTests/api/v1/ui/elements/UnitTests-home');
        ...
        if (!html.includes('<unittests-home')) {
    ...
    async testDataOperations() {
        ...
        const created = await this.apiCall('/apps/UnitTests/api/v1/data', {
            method: 'POST',
            body: createData
        });
        ...
        // Example: Read
        const read = await this.apiCall(`/apps/UnitTests/api/v1/data/${created.id}`);
        ...
        const updated = await this.apiCall(`/apps/UnitTests/api/v1/data/${created.id}`, {
            method: 'PUT',
            body: updateData
        });
        ...
        // Example: Delete
        await this.apiCall(`/apps/UnitTests/api/v1/data/${created.id}`, {
            method: 'DELETE'
        });
    ...
    async testErrorHandling() {
        ...
        try {
            await this.apiCall('/apps/UnitTests/api/v1/nonexistent');
...
if (typeof window.TestSuites !== 'undefined') {
    window.TestSuites.register(UnitTestsBrowserTests);
    console.log(`[TestSuite] Registered: ${UnitTestsBrowserTests.suiteName}`);
} else {
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = UnitTestsBrowserTests;
}

<#
.SYNOPSIS
    Initialization script for Redis Manager app
...
Write-Verbose "[RedisManager] Initializing Redis Manager app..."

# Create app namespace in global state
$PSWebServer['RedisManager'] = [hashtable]::Synchronized(@{
    AppRoot = $AppRoot
    DataPath = Join-Path $Global:PSWebServer['DataRoot'] "apps\RedisManager"
    Initialized = Get-Date
    # Add app-specific state here
})

# Ensure data directory exists
$dataPath = $PSWebServer.RedisManager.DataPath
...
#     $PSWebServer.RedisManager.Config = $config
# }
...
Write-Host "[Init] Loaded app: Redis Manager (v1.0.0)" -ForegroundColor Green

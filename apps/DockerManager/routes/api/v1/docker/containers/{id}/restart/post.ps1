# Restart Docker Container Endpoint
...
try {
    ...
    $result = Restart-DockerContainer -ContainerId $containerId

    # Log the operation
    if ($result.success) {
        Write-PSWebHostLog -Message "Container restarted: $containerId by $($sessiondata.Email)" -Level 'Info' -Facility 'DockerManager'
    }
    ...
    Write-PSWebHostLog -Message "Error restarting container: $($_.Exception.Message)" -Level 'Error' -Facility 'DockerManager'

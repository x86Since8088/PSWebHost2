# Get Docker Container Logs Endpoint
# Returns logs from a Docker container
...
try {
    ...
    if ($Request.QueryString['tail']) {
        try {
            ...
            # Invalid tail value, use default
            $tail = 100
        }
    }
    ...
    # Get container logs
    $result = Get-DockerContainerLogs -ContainerId $containerId -Tail $tail
    ...
    Write-PSWebHostLog -Message "Error getting container logs: $($_.Exception.Message)" -Level 'Error' -Facility 'DockerManager'

# Stop Docker Container Endpoint
...
try {
    ...
    # Stop the container
    $result = Stop-DockerContainer -ContainerId $containerId

    # Log the operation
    if ($result.success) {
        Write-PSWebHostLog -Message "Container stopped: $containerId by $($sessiondata.Email)" -Level 'Info' -Facility 'DockerManager'
    }
    ...
    Write-PSWebHostLog -Message "Error stopping container: $($_.Exception.Message)" -Level 'Error' -Facility 'DockerManager'

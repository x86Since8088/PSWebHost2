...
// Define test suite
const DockerManagerBrowserTests = {
    suiteName: 'DockerManager Browser Tests',
    version: '1.0.0',
    author: 'PSWebHost',
    ...
    async testComponentLoading() {
        const testElement = document.createElement('dockermanager-home');
    ...
    async testAPIEndpoint() {
        const response = await fetch('/apps/DockerManager/api/v1/status');
    ...
    async testUIElementRendering() {
        const response = await fetch('/apps/DockerManager/api/v1/ui/elements/DockerManager-home');
        ...
        if (!html.includes('<dockermanager-home')) {
    ...
    async testDataOperations() {
        ...
        const created = await this.apiCall('/apps/DockerManager/api/v1/data', {
            method: 'POST',
            body: createData
        });
        ...
        // Example: Read
        const read = await this.apiCall(`/apps/DockerManager/api/v1/data/${created.id}`);
        ...
        const updated = await this.apiCall(`/apps/DockerManager/api/v1/data/${created.id}`, {
            method: 'PUT',
            body: updateData
        });
        ...
        // Example: Delete
        await this.apiCall(`/apps/DockerManager/api/v1/data/${created.id}`, {
            method: 'DELETE'
        });
    ...
    async testLocalStorage() {
        const testKey = 'DockerManager_test';
    ...
    async testErrorHandling() {
        ...
        try {
            await this.apiCall('/apps/DockerManager/api/v1/nonexistent');
...
if (typeof window.TestSuites !== 'undefined') {
    window.TestSuites.register(DockerManagerBrowserTests);
    console.log(`[TestSuite] Registered: ${DockerManagerBrowserTests.suiteName}`);
} else {
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = DockerManagerBrowserTests;
}

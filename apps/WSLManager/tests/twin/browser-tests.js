...
// Define test suite
const WSLManagerBrowserTests = {
    suiteName: 'WSLManager Browser Tests',
    version: '1.0.0',
    author: 'PSWebHost',
    ...
    async testComponentLoading() {
        const testElement = document.createElement('wslmanager-home');
    ...
    async testAPIEndpoint() {
        const response = await fetch('/apps/WSLManager/api/v1/status');
    ...
    async testUIElementRendering() {
        const response = await fetch('/apps/WSLManager/api/v1/ui/elements/WSLManager-home');
        ...
        if (!html.includes('<wslmanager-home')) {
    ...
    async testDataOperations() {
        ...
        const created = await this.apiCall('/apps/WSLManager/api/v1/data', {
            method: 'POST',
            body: createData
        });
        ...
        // Example: Read
        const read = await this.apiCall(`/apps/WSLManager/api/v1/data/${created.id}`);
        ...
        const updated = await this.apiCall(`/apps/WSLManager/api/v1/data/${created.id}`, {
            method: 'PUT',
            body: updateData
        });
        ...
        // Example: Delete
        await this.apiCall(`/apps/WSLManager/api/v1/data/${created.id}`, {
            method: 'DELETE'
        });
    ...
    async testErrorHandling() {
        ...
        try {
            await this.apiCall('/apps/WSLManager/api/v1/nonexistent');
...
if (typeof window.TestSuites !== 'undefined') {
    window.TestSuites.register(WSLManagerBrowserTests);
    console.log(`[TestSuite] Registered: ${WSLManagerBrowserTests.suiteName}`);
} else {
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = WSLManagerBrowserTests;
}

<#
.SYNOPSIS
    Initialization script for MySQL Manager app
...
Write-Verbose "[MySQLManager] Initializing MySQL Manager app..."

# Create app namespace in global state
$PSWebServer['MySQLManager'] = [hashtable]::Synchronized(@{
    AppRoot = $AppRoot
    DataPath = Join-Path $Global:PSWebServer['DataRoot'] "apps\MySQLManager"
    Initialized = Get-Date
    # Add app-specific state here
})

# Ensure data directory exists
$dataPath = $PSWebServer.MySQLManager.DataPath
...
#     $PSWebServer.MySQLManager.Config = $config
# }
...
Write-Host "[Init] Loaded app: MySQL Manager (v1.0.0)" -ForegroundColor Green

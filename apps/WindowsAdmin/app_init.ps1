<#
.SYNOPSIS
    Initialization script for Windows Administration app
...
Write-Verbose "[WindowsAdmin] Initializing Windows Administration app..."

# Create app namespace in global state
$PSWebServer['WindowsAdmin'] = [hashtable]::Synchronized(@{
    AppRoot = $AppRoot
    DataPath = Join-Path $Global:PSWebServer['DataRoot'] "apps\WindowsAdmin"
    Initialized = Get-Date
    # Add app-specific state here
})

# Ensure data directory exists
$dataPath = $PSWebServer.WindowsAdmin.DataPath
...
#     $PSWebServer.WindowsAdmin.Config = $config
# }
...
Write-Host "[Init] Loaded app: Windows Administration (v1.0.0)" -ForegroundColor Green

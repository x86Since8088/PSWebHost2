...
// Define test suite
const WindowsAdminBrowserTests = {
    suiteName: 'WindowsAdmin Browser Tests',
    version: '1.0.0',
    author: 'PSWebHost',
    ...
    async testComponentLoading() {
        const testElement = document.createElement('windowsadmin-home');
    ...
    async testAPIEndpoint() {
        const response = await fetch('/apps/WindowsAdmin/api/v1/status');
    ...
    async testUIElementRendering() {
        const response = await fetch('/apps/WindowsAdmin/api/v1/ui/elements/WindowsAdmin-home');
        ...
        if (!html.includes('<windowsadmin-home')) {
    ...
    async testDataOperations() {
        ...
        const created = await this.apiCall('/apps/WindowsAdmin/api/v1/data', {
            method: 'POST',
            body: createData
        });
        ...
        // Example: Read
        const read = await this.apiCall(`/apps/WindowsAdmin/api/v1/data/${created.id}`);
        ...
        const updated = await this.apiCall(`/apps/WindowsAdmin/api/v1/data/${created.id}`, {
            method: 'PUT',
            body: updateData
        });
        ...
        // Example: Delete
        await this.apiCall(`/apps/WindowsAdmin/api/v1/data/${created.id}`, {
            method: 'DELETE'
        });
    ...
    async testLocalStorage() {
        const testKey = 'WindowsAdmin_test';
    ...
    async testErrorHandling() {
        ...
        try {
            await this.apiCall('/apps/WindowsAdmin/api/v1/nonexistent');
...
if (typeof window.TestSuites !== 'undefined') {
    window.TestSuites.register(WindowsAdminBrowserTests);
    console.log(`[TestSuite] Registered: ${WindowsAdminBrowserTests.suiteName}`);
} else {
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = WindowsAdminBrowserTests;
}

...
// Define test suite
const UI_UplotBrowserTests = {
    suiteName: 'UI_Uplot Browser Tests',
    version: '1.0.0',
    author: 'PSWebHost',
    ...
    async testComponentLoading() {
        const testElement = document.createElement('ui_uplot-home');
    ...
    async testAPIEndpoint() {
        const response = await fetch('/apps/UI_Uplot/api/v1/status');
    ...
    async testUIElementRendering() {
        const response = await fetch('/apps/UI_Uplot/api/v1/ui/elements/UI_Uplot-home');
        ...
        if (!html.includes('<ui_uplot-home')) {
    ...
    async testDataOperations() {
        ...
        const created = await this.apiCall('/apps/UI_Uplot/api/v1/data', {
            method: 'POST',
            body: createData
        });
        ...
        // Example: Read
        const read = await this.apiCall(`/apps/UI_Uplot/api/v1/data/${created.id}`);
        ...
        const updated = await this.apiCall(`/apps/UI_Uplot/api/v1/data/${created.id}`, {
            method: 'PUT',
            body: updateData
        });
        ...
        // Example: Delete
        await this.apiCall(`/apps/UI_Uplot/api/v1/data/${created.id}`, {
            method: 'DELETE'
        });
    ...
    async testErrorHandling() {
        ...
        try {
            await this.apiCall('/apps/UI_Uplot/api/v1/nonexistent');
...
if (typeof window.TestSuites !== 'undefined') {
    window.TestSuites.register(UI_UplotBrowserTests);
    console.log(`[TestSuite] Registered: ${UI_UplotBrowserTests.suiteName}`);
} else {
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = UI_UplotBrowserTests;
}

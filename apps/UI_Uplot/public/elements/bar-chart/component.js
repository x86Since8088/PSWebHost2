/**
 * uPlot Bar Chart Component
 * Bar charts for categorical and discrete data visualization
 */
...
class BarChartComponent extends HTMLElement {
    ...
    async connectedCallback() {
        ...
        this.logger.info(`Bar Chart Component loaded for chart: ${this.chartId}`);
    ...
    initializeLogger() {
        ...
        if (typeof ConsoleAPILogger !== 'undefined') {
            ...
            this.logger = { info: (...args) => console.log('[bar-chart:info]', ...args), warn: (...args) => console.warn('[bar-chart:warn]', ...args), error: (...args) => console.error('[area-chart:error]', ...args), debug: (...args) => console.debug('[bar-chart:debug]', ...args), verbose: (...args) => console.log('[bar-chart:verbose]', ...args) };
        }
    }
    ...
    async loadChartConfig() {
        try {
            ...
            this.chartConfig = {
                ...
                barWidth: parseFloat(params.get('barWidth')) || 0.6, // 0.0 to 1.0
                grouped: params.get('grouped') !== 'false', // grouped or stacked
                dataSource: {
    ...
    parseDataSourceConfig(params) {
        ...
        switch (sourceType) {
            ...
            case 'metrics-db':
                return {
                    metricName: params.get('metric') || 'request_count',
    ...
    async initializeChart() {
        this.logger.info('Initializing bar chart');
            ...
            axes: [
                {
                    ...
                    space: 80,
                    values: (u, vals, space) => {
                        // For categorical data, show labels
                        if (initialData.metadata?.categories) {
                            return vals.map(v => initialData.metadata.categories[Math.round(v)] || v);
                        }
        ...
        this.logger.info('Bar chart created successfully');
    ...
    buildSeriesConfig(metadata) {
        const series = [
            {
                // X-axis series
                label: metadata?.xAxisLabel || 'Categories'
            }
        ];
            ...
            series.push({
                ...
                paths: (u, seriesIdx, idx0, idx1) => {
                    return uPlot.paths.bars({
                        size: [this.chartConfig.barWidth, 100],
                        align: this.chartConfig.grouped ? 1 : 0, // 1 = grouped, 0 = stacked
                        gap: this.chartConfig.grouped ? 4 : 0
            ...
            <div class="bar-chart-container">
                ...
                <div class="chart-footer">
                    ...
                    <span class="chart-type-info">
                        <i class="fas fa-chart-bar"></i> ${this.chartConfig.orientation} bars (${this.chartConfig.grouped ? 'grouped' : 'stacked'})
                    </span>
                    ${this.chartConfig.realTime ? `
    ...
    convertToCSV(data) {
        ...
        // Header row
        let csv = 'Category,' + series.map((_, i) => `Series${i + 1}`).join(',') + '\n';

        // Data rows
        for (let i = 0; i < categories.length; i++) {
            const row = [categories[i], ...series.map(s => s[i])];
            csv += row.join(',') + '\n';
        }
    ...
    disconnectedCallback() {
        ...
        this.logger?.info('Bar Chart Component destroyed');
    }
}
...
if (typeof window.cardComponents !== 'undefined') {
    window.cardComponents['bar-chart'] = ({ element, url }) => {
        const React = window.React;
        const ref = React.useRef(null);

        React.useEffect(() => {
            if (ref.current && !ref.current.querySelector('bar-chart')) {
                const component = document.createElement('bar-chart');

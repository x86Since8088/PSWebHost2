...
// Define test suite
const KubernetesManagerBrowserTests = {
    suiteName: 'KubernetesManager Browser Tests',
    version: '1.0.0',
    author: 'PSWebHost',
    ...
    async testComponentLoading() {
        const testElement = document.createElement('kubernetesmanager-home');
    ...
    async testAPIEndpoint() {
        const response = await fetch('/apps/KubernetesManager/api/v1/status');
    ...
    async testUIElementRendering() {
        const response = await fetch('/apps/KubernetesManager/api/v1/ui/elements/KubernetesManager-home');
        ...
        if (!html.includes('<kubernetesmanager-home')) {
    ...
    async testDataOperations() {
        ...
        const created = await this.apiCall('/apps/KubernetesManager/api/v1/data', {
            method: 'POST',
            body: createData
        });
        ...
        // Example: Read
        const read = await this.apiCall(`/apps/KubernetesManager/api/v1/data/${created.id}`);
        ...
        const updated = await this.apiCall(`/apps/KubernetesManager/api/v1/data/${created.id}`, {
            method: 'PUT',
            body: updateData
        });
        ...
        // Example: Delete
        await this.apiCall(`/apps/KubernetesManager/api/v1/data/${created.id}`, {
            method: 'DELETE'
        });
    ...
    async testLocalStorage() {
        const testKey = 'KubernetesManager_test';
    ...
    async testErrorHandling() {
        ...
        try {
            await this.apiCall('/apps/KubernetesManager/api/v1/nonexistent');
...
if (typeof window.TestSuites !== 'undefined') {
    window.TestSuites.register(KubernetesManagerBrowserTests);
    console.log(`[TestSuite] Registered: ${KubernetesManagerBrowserTests.suiteName}`);
} else {
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = KubernetesManagerBrowserTests;
}

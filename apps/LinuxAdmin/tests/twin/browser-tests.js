...
// Define test suite
const LinuxAdminBrowserTests = {
    suiteName: 'LinuxAdmin Browser Tests',
    version: '1.0.0',
    author: 'PSWebHost',
    ...
    async testComponentLoading() {
        const testElement = document.createElement('linuxadmin-home');
    ...
    async testAPIEndpoint() {
        const response = await fetch('/apps/LinuxAdmin/api/v1/status');
    ...
    async testUIElementRendering() {
        const response = await fetch('/apps/LinuxAdmin/api/v1/ui/elements/LinuxAdmin-home');
        ...
        if (!html.includes('<linuxadmin-home')) {
    ...
    async testDataOperations() {
        ...
        const created = await this.apiCall('/apps/LinuxAdmin/api/v1/data', {
            method: 'POST',
            body: createData
        });
        ...
        // Example: Read
        const read = await this.apiCall(`/apps/LinuxAdmin/api/v1/data/${created.id}`);
        ...
        const updated = await this.apiCall(`/apps/LinuxAdmin/api/v1/data/${created.id}`, {
            method: 'PUT',
            body: updateData
        });
        ...
        // Example: Delete
        await this.apiCall(`/apps/LinuxAdmin/api/v1/data/${created.id}`, {
            method: 'DELETE'
        });
    ...
    async testErrorHandling() {
        ...
        try {
            await this.apiCall('/apps/LinuxAdmin/api/v1/nonexistent');
...
if (typeof window.TestSuites !== 'undefined') {
    window.TestSuites.register(LinuxAdminBrowserTests);
    console.log(`[TestSuite] Registered: ${LinuxAdminBrowserTests.suiteName}`);
} else {
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = LinuxAdminBrowserTests;
}

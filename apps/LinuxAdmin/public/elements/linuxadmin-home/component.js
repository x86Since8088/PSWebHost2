// Linux Administration Home Component
class LinuxAdminHome extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            status: null,
    ...
    async loadStatus() {
        try {
            const response = await window.psweb_fetchWithAuthHandling('/apps/linuxadmin/api/v1/status');
...
window.customElements.define('linuxadmin-home', class extends HTMLElement {
    connectedCallback() {
        ReactDOM.render(
            React.createElement(LinuxAdminHome),
            this
        );
    }
});

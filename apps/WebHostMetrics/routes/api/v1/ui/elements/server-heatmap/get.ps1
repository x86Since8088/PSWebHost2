...
try {
    ...
    if ($historyMinutes) {
        ...
        if ($Test) {
            ...
            Write-Host "Type: history" -ForegroundColor Yellow
            Write-Host "Minutes: $minutes" -ForegroundColor Yellow
            Write-Host "Record Count: $($response_data.recordCount)" -ForegroundColor Yellow
    ...
    if ($Test) {
        ...
        Write-Host "Content-Type: application/json" -ForegroundColor Gray
        Write-Host "`nResponse Data:" -ForegroundColor Cyan
        $systemStats | ConvertTo-Json -Depth 10 | Write-Host
        Write-Host "`n=== Summary ===" -ForegroundColor Cyan
        Write-Host "Timestamp: $($systemStats.timestamp)" -ForegroundColor Yellow
        Write-Host "Hostname: $($systemStats.hostname)" -ForegroundColor Yellow
        Write-Host "Cached: $($systemStats.cached)" -ForegroundColor Yellow
        if ($systemStats.metrics.cpu) {
            Write-Host "CPU Cores: $($systemStats.metrics.cpu.Count)" -ForegroundColor Yellow
        }
        Write-Host "`n=== End Test Results ===" -ForegroundColor Cyan
        return
    }

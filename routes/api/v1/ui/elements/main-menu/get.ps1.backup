...
if ($search -match '^regex:') {
    ...
    if ($SearchRegexArr.Count -eq 0) { 
        $SearchRegexArr += '.*' 
    }
    else{
        Write-PSWebHostLog -Message "main-menu SearchRegexArr: $($SearchRegexArr -join '; ')" -Severity Verbose -Category MainMenu -Source $MyInvocation.MyCommand.Source
    }
}

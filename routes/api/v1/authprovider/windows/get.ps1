...
if ($isSessionValid -and ($SessionData.ID -match '\S{4,256}')) {
    ...
    Write-Host "`t[authprovider\windows\get.ps1] isSessionValid: $isSessionValid `n`t`t$(($SessionData|Inspect-Object | ConvertTo-YAML) -split '`n' -notmatch '^\s*Type:' -join "`n`t`t")"
    context_reponse -Response $Response -StatusCode 302 -RedirectLocation $redirectUrl
    return
}
else {
    Write-Host "`t[authprovider\windows\get.ps1] isSessionValid: $isSessionValid `n`t`t$(($SessionData|Inspect-Object -Depth 4| ConvertTo-YAML) -split '`n' -notmatch '^\s*Type:' -join "`n`t`t")"
}

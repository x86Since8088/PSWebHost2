...
if ([string]::IsNullOrEmpty($refGuid)) {
    context_response -Response $Response -StatusCode 400 -String "Missing reference GUID."
    return
}
...
if (-not $confirmationRequest) {
    context_response -Response $Response -StatusCode 404 -String "Invalid confirmation link."
    return
}

if ($confirmationRequest.response_date) {
    context_response -Response $Response -String "This email address has already been confirmed."
    return
}
...
if ($confirmationRequest.request_ip -ne $responseIp -or $confirmationRequest.request_session_id -ne $responseSessionId) {
    ...
    context_response -Response $Response -StatusCode 403 -String $message
    return
}
...
context_response -Response $Response -String $successMessage
#Requires -Version 7
# Card Validation with Authentication
# Logs in first, then tests all cards via HTTP endpoints
...
Write-Host "  Card Validation with Authentication" -ForegroundColor Cyan
Write-Host "=====================================================================" -ForegroundColor Cyan
Write-Host ""

# Step 1: Test server
Write-Host "[1/6] Testing Server..." -ForegroundColor Yellow

try {
    $response = Invoke-WebRequest -Uri $baseUrl -UseBasicParsing -Method HEAD -TimeoutSec 10 -MaximumRedirection 0 -ErrorAction SilentlyContinue
    Write-Host "  [OK] Server responding on port $Port (Status: $($response.StatusCode))" -ForegroundColor Green
} catch {
    if ($_.Exception.Response.StatusCode -eq 302) {
        Write-Host "  [OK] Server responding on port $Port (302 Redirect - Auth required)" -ForegroundColor Green
    } else {
...
# Step 2: Login
Write-Host ""
Write-Host "[2/6] Attempting Login..." -ForegroundColor Yellow
...
try {
    ...
    if ($loginResponse.StatusCode -eq 200) {
        ...
        Write-Host "  [WARN] Login status: $($loginResponse.StatusCode)" -ForegroundColor Yellow
    }
} catch {
...
# Step 3: Get main menu
Write-Host ""
Write-Host "[3/6] Fetching Main Menu..." -ForegroundColor Yellow

try {
    $menuResponse = Invoke-WebRequest `
        -Uri "$baseUrl/api/v1/ui/elements/main-menu" `
        -UseBasicParsing `
        -TimeoutSec 30 `
        -MaximumRedirection 5 `
        -WebSession $session

    if ($menuResponse.StatusCode -eq 200) {
        $menuData = $menuResponse.Content | ConvertFrom-Json
        Write-Host "  [OK] Main menu retrieved" -ForegroundColor Green
    } else {
        Write-Host "  [FAIL] Unexpected status: $($menuResponse.StatusCode)" -ForegroundColor Red
        exit 1
    }
} catch {
...
# Step 4: Extract all card URLs
Write-Host ""
Write-Host "[4/6] Extracting Card URLs..." -ForegroundColor Yellow
...
Write-Host ""
Write-Host "[5/6] Testing Card Endpoints..." -ForegroundColor Yellow
Write-Host ""

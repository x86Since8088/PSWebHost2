...
Import-Module (Join-Path $ProjectRoot 'tests\modules\TestCodeHelpers.psm1') -ErrorAction Continue -DisableNameChecking
...
try {
	...
	Assert-True -Condition (Test-Path (Split-Path $safe.path -Parent -ErrorAction Ignore) -ErrorAction Ignore) -Message 'Sanitize-FilePath returns valid path' | Out-Null

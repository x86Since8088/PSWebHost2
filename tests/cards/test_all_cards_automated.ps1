...
# Discover all cards - look in project root routes/cards and apps/*/routes/cards
Write-Host "`nDiscovering UI cards..." -ForegroundColor Yellow
$projectRoot = Join-Path $PSScriptRoot "..\.."
$cardPaths = @()

# Find cards in routes/cards
$coreCards = Get-ChildItem -Path (Join-Path $projectRoot "routes\cards") -Recurse -Filter "get.ps1" -ErrorAction SilentlyContinue |
    ForEach-Object {
        $relativePath = $_.FullName -replace [regex]::Escape($projectRoot), ""
        $relativePath = $relativePath -replace "\\", "/"

        # Extract card name from path: routes/cards/{cardname}/get.ps1 or routes/cards/{parent}/{cardname}/get.ps1
        if ($relativePath -match "/cards/([^/]+)/get\.ps1$") {
            $cardName = $matches[1]
            [PSCustomObject]@{
                Name = $cardName
                Path = $relativePath
                Endpoint = "/cards/$cardName"
                FilePath = $_.FullName
                App = "Core"
            }
        } elseif ($relativePath -match "/cards/([^/]+)/([^/]+)/get\.ps1$") {
            $cardName = "$($matches[1])/$($matches[2])"
            [PSCustomObject]@{
                Name = $cardName
                Path = $relativePath
                Endpoint = "/cards/$cardName"
...
# Find cards in apps/*/routes/cards
$appCards = Get-ChildItem -Path (Join-Path $projectRoot "apps") -Recurse -Filter "get.ps1" -ErrorAction SilentlyContinue |
    Where-Object { $_.FullName -match "routes[\\\/]cards" } |
    ForEach-Object {
        $relativePath = $_.FullName -replace [regex]::Escape($projectRoot), ""
        $relativePath = $relativePath -replace "\\", "/"

        # Extract app and card name
        if ($relativePath -match "/apps/([^/]+)/routes/cards/([^/]+)/get\.ps1$") {
            $appName = $matches[1]
            $cardName = $matches[2]
            [PSCustomObject]@{
                Name = $cardName
                Path = $relativePath
                Endpoint = "/apps/$appName/cards/$cardName"
                FilePath = $_.FullName
                App = $appName
            }
        }
...
$cardPaths = @($coreCards) + @($appCards) | Where-Object { $_ -ne $null -and $_.Name -like $CardFilter } | Sort-Object App, Name

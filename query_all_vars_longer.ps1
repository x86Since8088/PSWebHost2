...
foreach ($varName in $variables) {
    try {
        Write-Host "Querying $varName..." -ForegroundColor Cyan
        $body = @{
            command = "try{($varName | convertto-json -depth 15).length}catch{`$_.Exception.message}"
        } | ConvertTo-Json

        $result = Invoke-RestMethod -Uri 'http://localhost:8080/api/v1/cli' -Method Post -Headers $headers -Body $body -TimeoutSec 60

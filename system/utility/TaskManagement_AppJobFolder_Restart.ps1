...
<#
.SYNOPSIS
    Restarts a job from an app's jobs folder

.DESCRIPTION
    Restarts a job (stops then starts) using the new PSWebHost_Jobs module
...
.PARAMETER JobName
    The name of the job to restart

.PARAMETER UserID
    The user ID restarting the job (default: current user or 'system')
...
.EXAMPLE
    .\TaskManagement_AppJobFolder_Restart.ps1 -AppName "WebHostMetrics" -JobName "CollectMetrics"

.EXAMPLE
    .\TaskManagement_AppJobFolder_Restart.ps1 -AppName "WebHostMetrics" -JobName "CollectMetrics" -Variables @{Interval=60}
#>
...
$MyTag = '[TaskManagement:AppJobFolder:Restart]'
...
try {
    ...
    Write-Host "$MyTag Restarting job: $jobID" -ForegroundColor Cyan
    ...
    # Restart the job
    $result = Restart-PSWebHostJob -JobID $jobID -UserID $UserID -SessionID $SessionID -Variables $Variables -Roles $Roles

    Write-Host ""
    Write-Host "$MyTag Restart command queued successfully!" -ForegroundColor Green
    Write-Host "$MyTag Status: $($result.Status)" -ForegroundColor Green
    Write-Host ""

    # Process command queue to actually restart the job

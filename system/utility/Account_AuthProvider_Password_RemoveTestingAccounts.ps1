...
# Removes test accounts (accounts with email starting with TA_Password_)
...
$getAccountScript = Join-Path $PSScriptRoot "Account_AuthProvider_Password_Get.ps1"
...
$testAccounts | Format-Table UserID, Email, UserName, ProviderCreatedDate -AutoSize
...
$removeAccountScript = Join-Path $PSScriptRoot "Account_AuthProvider_Password_Remove.ps1"
$deletedCount = 0
$failedCount = 0

foreach ($account in $accountsToDelete) {
    try {
        Write-Host "`nDeleting: $($account.Email)..." -ForegroundColor Yellow

        if ($Force) {
            & $removeAccountScript -ID $account.UserID -Force -Confirm:$false
        }
        else {
            & $removeAccountScript -ID $account.UserID
        }

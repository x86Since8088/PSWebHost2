...
foreach ($localUser in $localUsers) {
    ...
    if (-not $existingUser) {
        ...
        try {
            ...
            Register-PSWebHostUser -UserName $localUser.Name -Email $localUser.Name -Provider "Password" -Password $randomPassword -ErrorAction Stop
            Write-Verbose "Successfully created user '$($localUser.Name)'."
        } catch {
            Write-Error "Failed to create user '$($localUser.Name)'. Error: $($_.Exception.Message)"
        }
    } else {
